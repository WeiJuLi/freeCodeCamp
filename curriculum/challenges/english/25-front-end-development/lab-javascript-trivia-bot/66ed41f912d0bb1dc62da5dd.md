...
```js
const codeWithoutComments = __helpers.removeJSComments(code);
const codingFactRegex = /(let )?\s*codingFact\s*=\s*(["']?).*?\2\s*\+\s*favoriteLanguage(\s*\+\s*(["']?).*?\4)?|(let )?\s*codingFact\s*=\s*favoriteLanguage\s*\+\s*(["']?).*?\6/g
const [first, second, third] = codeWithoutComments.match(codingFactRegex)
assert.match(code, codingFactRegex)
